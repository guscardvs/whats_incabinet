...
class ListItems(View, LoginRequiredMixin):
    ...
    def post(self, request):
        ...
        if (
            len(
                profile.item.filter(
                    item=core_models.Item.objects.get(name=form["name"])
                )
            )
            != 0
        ):
        ...
        item = core_models.UserItem.create_user_item(**core_utils.format_form(form))
        if item is not None:
            item.save()
...
class EditItem(View, LoginRequiredMixin):
    ...
    def get(self, request, pk):
        context = {"item": self.item(request, pk)}
        return render(request, self.template_name, context)
